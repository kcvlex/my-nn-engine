...
A comprehensive toolkit for debugging and testing ONNX models. This tool extracts subgraphs from ONNX models, runs them to generate expected outputs, and provides automated binary search to find failing nodes.
...
**Note**: This tool runs Python scripts in a Podman container. No Python installation needed on the host!
...
```bash
# Build once (or use cargo run -p onnx-test-tools -- to skip building)
cargo build -p onnx-test-tools --release
...
# Step 2: Run binary search via container
cargo run -p onnx-test-tools -- --binary-search \
  --model models/validated/bertsquad-12/bertsquad-12.onnx \
  --inputs models/validated/bertsquad-12/test_data_set_0/input_*.pb \
...
Examples:
```bash
# Using the built-in test_extracted example
--test-command ./target/release/examples/test_extracted "{extract_dir}" 0.01
...
To modify the extraction scripts:

1. Edit `extract_model.py` or `binary_search.py`
2. Rebuild the Podman image:
   ```bash
   cargo run -p onnx-test-tools -- --rebuild --help
   ```
3. Test your changes
